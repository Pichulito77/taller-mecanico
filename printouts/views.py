...
def render_to_pdf(template_name: str, context: dict) -> HttpResponse:
	# Lazy import to avoid import-time failures when reportlab/xhtml2pdf
...
def pdf_ot(request, pk: int):
	...
	return render_to_pdf(
		"printouts/ot_print.html", {"ot": ot, "variant": request.GET.get("v", "empresa")}
	)
...
def pdf_quote(request, pk: int):
	qt = get_object_or_404(Quote, pk=pk)
	return render_to_pdf(
		"printouts/quote_print.html", {"quote": qt, "variant": request.GET.get("v", "empresa")}
	)
...
def pdf_invoice(request, pk: int):
	inv = get_object_or_404(Invoice, pk=pk)
	return render_to_pdf(
		"printouts/invoice_print.html", {"invoice": inv, "variant": request.GET.get("v", "empresa")}
	)
